    ...
    public function __construct() {
       ...
       public function widget($args, $instance){ 
        ...
        if(!empty($instance["loadscripts"])) {$loadscripts = 'loadscripts='.$instance["loadscripts"];} else {$loadscripts = '';}
          ...
          <?php echo $before_widget;
           echo do_shortcode('[gmap address="'.$location.'" title="'.$locationtitle.'" '.$height.' '.$maptype.' '.$zoom. ' '.$scrollwheel.' '.$location2.' '.$location3.' '.$location4.' '.$center.' '.$locationtitle2.' '.$locationtitle3.' '.$locationtitle4.' '.$loadscripts.']');
           echo $after_widget;?>
