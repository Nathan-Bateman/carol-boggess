...
add_action( 'after_switch_theme', 'kf_reflush_rules' );
 add_action('kadence_gallery_album_before', 'kad_gallery_container_open');
  ...
  function kad_gallery_container_open() {
        ...
        echo '<div class="main '.kadence_main_class().' postclass" role="main">';
  }
...
function kad_save_extra_profile_fields( $user_id ) {
    if ( !current_user_can( 'edit_posts', $user_id ) )
        return false;
  	update_user_meta( $user_id, 'occupation', esc_textarea($_POST['occupation']) );
   	update_user_meta( $user_id, 'twitter', esc_attr($_POST['twitter']) );
  	update_user_meta( $user_id, 'facebook', esc_url($_POST['facebook']) );
  	update_user_meta( $user_id, 'google', esc_url($_POST['google']) );
  	update_user_meta( $user_id, 'youtube', esc_url($_POST['youtube']) );
  	update_user_meta( $user_id, 'flickr', esc_url($_POST['flickr']) );
  	update_user_meta( $user_id, 'vimeo', esc_url($_POST['vimeo']) );
  	update_user_meta( $user_id, 'linkedin', esc_url($_POST['linkedin']) );
  	update_user_meta( $user_id, 'dribbble', esc_url($_POST['dribbble']) );
  	update_user_meta( $user_id, 'pinterest', esc_url($_POST['pinterest']) );
